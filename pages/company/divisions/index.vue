...
import type { Division } from '~/stores/company'
import { Plus, Edit, Trash2, X, Check, AlertTriangle, Layers } from 'lucide-vue-next'

definePageMeta({ middleware: 'admin' })
useHead({ title: 'Divisi - Company' })
...
function openDelete(item: Division) {
  selected.value = item
...
function handleCreate() {
  store.addDivision({ name: form.value.name.trim(), code: form.value.code.trim(), description: form.value.description.trim() || undefined })
  closeModals()
}
function handleUpdate() {
  if (!selected.value) return
  store.updateDivision(selected.value.id, { name: form.value.name.trim(), code: form.value.code.trim(), description: form.value.description.trim() || undefined })
  closeModals()
}
function handleDelete() {
  if (!selected.value) return
  store.deleteDivision(selected.value.id)
  closeModals()
}
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-foreground">Divisi</h1>
        <p class="text-muted-foreground">Kelola data divisi</p>
      </div>
    ...
    <div v-if="store.divisions.length === 0" class="glass-card p-12 text-center">
      <Layers class="w-12 h-12 mx-auto text-muted-foreground mb-4" />
      <h3 class="text-lg font-semibold text-foreground mb-2">No Divisi</h3>
      <p class="text-muted-foreground mb-4">Tambahkan divisi pertama.</p>
      <UiButton variant="gradient" @click="openCreate"><Plus class="w-4 h-4" /> Add Divisi</UiButton>
    </div>
    ...
    <div v-else class="rounded-xl border border-border overflow-hidden bg-card/50">
      <div class="overflow-x-auto">
        <table class="w-full min-w-max">
          ...
          <tbody>
            <tr v-for="item in store.divisions" :key="item.id" class="border-b border-border last:border-0 hover:bg-muted/30">
        ...
        <div v-if="showCreateModal || showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative w-full max-w-md bg-card border border-border rounded-2xl p-6">
            <div class="flex justify-between items-center mb-4">
              <h2 class="text-lg font-bold">{{ showCreateModal ? 'Add Divisi' : 'Edit Divisi' }}</h2>
              <button class="p-2 rounded-lg hover:bg-muted/50" @click="closeModals"><X class="w-5 h-5" /></button>
            </div>
            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium mb-2">Name *</label>
                <UiInput v-model="form.name" placeholder="Divisi name" />
              </div>
              <div>
                <label class="block text-sm font-medium mb-2">Code *</label>
                <UiInput v-model="form.code" placeholder="e.g. SD" />
              </div>
              <div>
      ...
      <Transition enter-active-class="transition duration-200" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative w-full max-w-md bg-card border border-border rounded-2xl p-6 text-center">
            ...
            <h2 class="text-lg font-bold mb-2">Delete Divisi</h2>
