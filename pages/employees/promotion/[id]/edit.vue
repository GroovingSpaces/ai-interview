...
onMounted(() => {
  ...
  form.value = {
    ...
    notes: item.notes ?? '',
    promotedById: item.promotedById ?? '',
    documents: (item.documents ?? []).map((d) => ({ name: d.name, fileName: d.fileName, fileData: d.fileData })),
  }
})
</script>
    ...
    <form class="space-y-6 w-full" @submit.prevent="handleSubmit">
      <div class="glass-card p-6 space-y-6">
        ...
        <div>
          ...
          <div v-if="form.documents.length === 0" class="text-sm text-muted-foreground py-2">{{ t('noDocuments') || 'Belum ada dokumen.' }}</div>
          <div v-else class="space-y-3">
            ...
            >
              ...
              <div class="flex-1 space-y-2">
                <UiInput v-model="doc.name" placeholder="Nama dokumen" />
                <input
                  type="file"
              ...
              <button type="button" class="p-2 rounded-lg hover:bg-score-low/10 text-score-low" @click="removeDocument(idx)">
