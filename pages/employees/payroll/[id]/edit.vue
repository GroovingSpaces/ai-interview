...
import { ArrowLeft, Plus, Check, Trash2 as TrashIcon } from 'lucide-vue-next'
...
const payroll = computed(() => payrollStore.getPayrollById(payrollId.value))

useHead({ title: () => `${t('editPayrollModal')} | ${title.value}` })
...
function handleSubmit() {
  ...
  payrollStore.updatePayroll(item.id, {
    employeeId: form.value.employeeId,
    periodMonth: form.value.periodMonth,
    periodYear: Number(form.value.periodYear) || item.periodYear,
  ...
  form.value = {
    ...
    allowanceItems:
      item.allowances?.length > 0
        ? item.allowances.map((a) => ({ id: a.id, name: a.name, amount: a.amount }))
        : [],
    bpjsTk: item.bpjsTk ?? 0,
    bpjsKesehatan: item.bpjsKesehatan ?? 0,
    otherDeductions: item.otherDeductions ?? 0,
    ptkpStatus: item.ptkpStatus ?? 'TK/0',
    taxableIncome: item.taxableIncome ?? 0,
    pph21: item.pph21 ?? 0,
    status: item.status,
    notes: item.notes ?? '',
  }
  if (form.value.allowanceItems.length === 0) addAllowanceRow()
})
</script>

<template>
  <div v-if="payroll" class="space-y-6">
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-2xl font-bold text-foreground">{{ t('editPayrollModal') }}</h1>
