...
useHead({ title: () => 'Edit Position | Position Management' })

const route = useRoute()
const router = useRouter()
const appStore = useApplicationStore()
const positionId = computed(() => route.params.id as string)
const position = computed(() => appStore.getPositionById(positionId.value))
...
function handleSubmit() {
  ...
  if (!item) return
  appStore.updatePosition(item.id, {
...
<template>
  <div v-if="position" class="space-y-6">
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-2xl font-bold text-foreground">Edit Position</h1>
